...
在第二阶段已有 RAG 服务基础上增加 Agent，对外提供多轮对话和工具调用能力，面向最终用户。
...
### 6.4 Agent 与 RAG 的职责边界

| RAG 管理 | Agent 管理 |
|----------|------------|
| Query rewrite | 多轮任务目标 |
| retrieval context | 工具调用历史 |
| 检索过滤条件 | 当前任务阶段 |
| 上下文拼装 | 任务级决策上下文 |
...
| 工具 | 说明 |
|------|------|
| `rag_search_tool` | 调用 `/rag/search` |
| `rag_answer_tool` | 调用 `/rag/answer` |
| `document_lookup_tool` | 查询文档信息、知识库状态 |
| `sql_tool` | 预留 |
| `api_tool` | 预留 |
| `rule_tool` | 预留 |
...
| 接口 | 方法 | 说明 |
|------|------|------|
| `/agent/chat` | POST | Agent 对话 |
| `/agent/sessions` | GET | 会话列表 |
| `/agent/sessions/{session_id}` | GET | 会话详情 |
| `/agent/trace` | GET | trace 查询 |

### 6.7 前端页面

| 页面 | 说明 |
|------|------|
| Agent 对话页 | 多轮聊天界面、流式输出、工具调用过程展示、引用来源展示、trace 折叠面板、会话列表 |
...
| M5（第三阶段） | Agent 页面与工具调用打通 |
| M6（第三阶段） | 多轮对话和 trace 展示打通 |
