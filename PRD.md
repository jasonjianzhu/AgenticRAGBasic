...
```
用户 query
  → 1. Normalize（全角转半角、空白清理）
  → 2. Rewrite（可选，LLM 改写扩展 query）
  → 3. Embed（原始 query embedding；若开启 rewrite，改写 query 也做 embedding）
  → 4. Search（每个 KB 分别做 dense + sparse 检索）
  → 5. RRF Fusion（合并 dense 和 sparse 结果，计算 RRF score）
  → 6. RRF Score Threshold 过滤（去除低质量噪音候选）
  → 7. Rerank（可选，用原始 query 对候选集精排，只排序不过滤）
  → 8. 取 top_k
  → 9. Enrich（从 PostgreSQL 补充文档标题、页码等元数据）
  → 返回结果
```
...
```
Query → Normalize → [Rewrite] → Embed → Dense+Sparse Search → RRF Fusion
  → Threshold Filter → [Rerank] → Top-K → Enrich → Return
```
