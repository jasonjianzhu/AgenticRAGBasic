...
- LLM 调用通过统一抽象接口（`BaseLLMClient`），切换模型只需新增实现类 + 修改配置
- 支持的 LLM 接入方式：OpenAI-compatible API（MiniMax、DeepSeek、Moonshot 等）、Anthropic API、本地部署（vLLM/Ollama）
- Agent 和 RAG 可配置使用不同模型（如 Agent 用强模型、RAG rewrite 用轻量模型）
- Embedding 和 Reranker 同样通过抽象接口隔离，支持本地模型和远程服务切换
- 模型切换不需要改代码，只需修改 `.env` 配置
...
- [ ] request_id 贯穿全链路日志
- [ ] 操作审计日志可查
- [ ] MinIO 对象存储接入，文件存储切换完成
- [ ] 评测体系可运行（Hit@K、MRR、Groundedness）
...
| M7（第四阶段） | 鉴权、RBAC、限流、审计、观测链路打通 |
| M8（第四阶段） | MinIO 接入、LLM 可替换、评测体系、用户反馈、导出、多 worker 部署 |
