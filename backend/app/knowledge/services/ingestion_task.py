...
from app.common.db.models import Document, DocumentVersion, JobLog
from app.knowledge.rag.chunking.base import ChunkData
from app.knowledge.rag.chunking.registry import default_registry
from app.knowledge.rag.classification.rule_based import RuleBasedClassifier
from app.knowledge.rag.parsing.base import ParsedDocument
from app.knowledge.rag.parsing.docling_parser import DoclingParser
...
class IngestionTaskService:
    ...
    def _save_parsed_output(
        ...
        import json
        from app.knowledge.rag.parsing.serialization import parsed_json_path
    ...
    def _run_chunking(self, parsed: ParsedDocument, doc: Document) -> list[ChunkData]:
        """Run the appropriate chunker + table chunker, then assign page numbers."""
