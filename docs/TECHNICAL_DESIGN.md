...
| 版本 | 日期 | 作者 | 说明 |
|------|------|------|------|
| v0.3 | 2026-04-26 | AgenticRAG Team | 第四阶段产品化重写；补充 RAG pipeline、Agent prompt 工程、检索优化设计 |
...
graph TB
    subgraph 前端
        F1[Frontend<br/>RAG 问答页面<br/>Agent 对话页面]
        F2[Admin Frontend<br/>文档上传<br/>解析/Chunk/索引管理]
    end
    ...
    subgraph 存储层
        PG[(PostgreSQL<br/>文档/任务/会话/配置/审计/版本)]
        QD[(Qdrant<br/>dense + sparse)]
    end

    subgraph Worker
        W[Worker<br/>parse / chunk / embed / index]
    end
...
| failed | 失败（可重试） |
| disabled | 人工禁用 |
| low_quality | 解析质量不达标（chunk 为空、乱码等），第四阶段引入 |
...
- 文档版本清理：创建新版本前，清理旧版本的 chunks 和 Qdrant 向量，避免多版本混合（第四阶段完善）
...
- 扫描件自动检测，提示用户切换到 accurate profile
- 图片 caption 提取（当前未实现，第四阶段补充）

#### 4.2.4 Chunk 构建（Chunker Registry）

| Chunker | 说明 |
|---------|------|
| docling_hybrid | 结构感知 + token 限制（默认） |
...
| 支持文件类型 | PDF（第一~三阶段） | 第四阶段可扩展 DOCX、Markdown、TXT |
...
#### 5.2.5 RAG 检索 Pipeline

完整的检索流程（单次 search 调用）：

```mermaid
graph LR
    Q[用户 Query] --> N[1. Normalize<br/>全角转半角/空白清理]
    N --> R{Rewrite<br/>开启?}
    R -->|是| RW[2. LLM Rewrite]
    R -->|否| E1
    RW --> E2[3. Embed 改写 Query]
    N --> E1[3. Embed 原始 Query]
    E1 --> S[4. Dense + Sparse Search<br/>每个 KB 分别检索]
    E2 --> S
    S --> RRF[5. RRF Fusion<br/>合并打分]
    RRF --> TH[6. RRF Threshold 过滤<br/>去噪]
    TH --> RK{Rerank<br/>开启?}
    RK -->|是| RERANK[7. Rerank 精排<br/>用原始 Query]
    RK -->|否| TOP
    RERANK --> TOP[8. 取 Top-K]
    TOP --> EN[9. Enrich<br/>补充元数据]
    EN --> RET[返回结果]
```

设计原则：
- **过滤在排序之前**：RRF threshold 在 rerank 之前执行，reranker 只负责排序不负责过滤
- **双路检索**（开启 rewrite 时）：原始 query 和改写 query 同时检索，RRF 合并，只增加召回不丢失精确匹配
- **Reranker 用原始 query 评分**：reranker 判断的是用户原始意图和文档的相关性，不用改写后的 query
...
第四阶段增强：
- RAG 问答页增加过滤条件选择器（文档类型、语言、产品型号），依赖 chunk 元数据补全
- Query Rewrite 同义扩充：在 rewrite 阶段输出同义词扩展，利用 sparse retrieval 增加召回
...
```mermaid
graph LR
    U[用户问题] --> A[Agent 判断需要查数据]
    A --> G[LLM 根据 Schema 生成 SQL]
    G --> V[应用层校验<br/>只读 + 白名单 + 行数限制]
    V --> E[执行查询, 返回结果]
    E --> S[Agent 生成文字总结<br/>或调用 chart_tool 生成图表]
```
...
| 配置项 | 默认值 | 说明 |
|--------|--------|------|
| 会话过期时间 | 24 小时 | 超时自动归档（第四阶段实现定时清理任务） |
...
Agent system prompt 中包含严格的回答原则，防止 LLM 编造信息：

- **禁止编造任何数据和信息**：所有数字、数据、参数必须来自工具调用的返回结果
- **知识问答必须基于检索结果**：`rag_search` 返回为空时，告知用户"知识库中暂未找到"，不用自身知识回答
- **数据必须来自 sql_query**：涉及具体数值时必须先查询，查不到就说"暂无数据"
- **只回答用户问的问题**：不主动补充用户没问的参数，不列出"未找到 XXX"

#### 6.6.2 工具调用约束

- `rag_search` 的 query 必须原封不动使用用户原始问题，禁止 Agent 自行改写
- Agent 调用 `rag_search` 时关闭 RAG 侧的 query rewrite（`enable_rewrite=False`），避免双重改写
- 工具调用次数限制：`rag_search` 最多 3 次、`sql_query` 最多 2 次、`generate_chart` 最多 1 次

#### 6.6.3 Citation 精确匹配

- `rag_search` 返回的所有 chunk 暂存为候选 citation，不立即发送给前端
- Agent 回答完成后，解析回答文本中的 `[1]` `[2]` 引用标记
- 只发送被实际引用的 citation 给前端，避免不相关的引用来源干扰用户

#### 6.6.4 Prompt 管理（第四阶段）

- Agent system prompt 和 RAG answer prompt 从代码中抽出，支持配置化或模板化
...
当前 Docling 解析时跳过了文档中的图片（示意图、接线图、流程图等），用户搜索相关内容时只能看到"图片在第 X 页"的文字提示，无法直接展示图片。

**解析阶段**：
- 识别 Docling 输出的 PictureItem，提取图片二进制数据（PIL Image）
- 图片保存到文件存储（本地/MinIO），路径格式：`{kb_id}/{doc_id}/images/{image_id}.png`
- 提取图片 caption（Docling 自带）和周围文本上下文

**数据模型**：
- `ParsedDocument` 新增 `images` 字段（图片路径、页码、caption、上下文文本）
- `ChunkData` 支持 `chunk_type="image"`，content 存 caption + 上下文描述，metadata 存图片存储路径
- Chunk 表新增 `image_path` 字段（可选）

**索引**：
- 图片 chunk 的 embedding 用 caption + 周围文本生成（文本 embedding）
- Qdrant payload 携带 `image_path`

**检索展示**：
- 搜索命中图片 chunk 时，返回图片 URL
- 图片服务接口：`GET /images/{kb_id}/{doc_id}/images/{filename}`
- 前端在引用来源和回答中渲染图片

**可选增强**：
- 用多模态模型（GPT-4V / Qwen-VL）生成图片描述，替代简单 caption，提升检索语义匹配质量
...
- [ ] 多 worker 部署验证通过
- [ ] 文档图片提取、索引、检索展示可用
- [ ] 文档版本清理（旧版本 chunks 自动清理）
- [ ] 切分逻辑优化（利用文档结构树）
- [ ] Alembic 迁移 autogenerate 验证通过
- [ ] 文档类型分类准确率提升
- [ ] RAG 问答页过滤条件可用
- [ ] 会话过期自动清理
- [ ] 会话标题智能生成
...
| 测试类型 | 说明 |
|----------|------|
| 单元测试 | 纯逻辑，不依赖外部服务。覆盖率目标 >70% |
| 契约测试 | 测试适配器边界（Parser、Client、Repository） |
| 集成测试 | 关键链路端到端测试（含 2-3 个真实 PDF） |
| 评测测试 | 检索效果和答案质量回归 |

重点补充测试的模块：
- RAG 服务核心逻辑（query 处理、RRF 融合、拒答策略）
- SQL validator、chart tool、session service
- 前端 Agent 组件测试
...
```mermaid
graph TB
    P1[第一阶段<br/>知识库平台] --> P2[第二阶段<br/>RAG 知识问答]
    P2 --> P3[第三阶段<br/>Agent 构建]
    P3 --> P4[第四阶段<br/>产品化]
```
...
### 9.5 RAG 检索 Pipeline 技术方案

完整的检索 pipeline 经过三阶段联调优化，最终确定为以下流程：
...
graph LR
    ...
    N --> RW["[Rewrite]"]
    RW --> E[Embed]
    E --> S[Dense + Sparse Search]
    S --> RRF[RRF Fusion]
    RRF --> TF[Threshold Filter]
    TF --> RK["[Rerank]"]
    RK --> TK[Top-K]
    TK --> EN[Enrich]
    EN --> R[Return]
...
| 决策 | 说明 | 原因 |
|------|------|------|
| RRF threshold 在 rerank 之前 | 先过滤再排序 | 过滤和排序职责分离，reranker 不负责过滤 |
| 双路检索（开启 rewrite 时） | 原始 query + 改写 query 同时检索 | 改写只增加召回不丢失精确匹配 |
| Reranker 用原始 query | 不用改写后的 query 做 rerank | reranker 判断用户原始意图与文档的相关性 |
| Agent 关闭 RAG rewrite | Agent 调 RAG 时 `enable_rewrite=False` | 避免 Agent LLM 改写 + RAG rewrite 双重改写导致语义偏移 |
| RRF score threshold = 0.012 | 过滤单列表排名 20+ 的噪音 | RRF score 范围约 0.01-0.033（k=60），0.01 形同虚设 |

RRF Score 参考值（k=60）：

| 场景 | Score |
|------|-------|
| 同时在 dense 和 sparse 排名第 1 | ~0.033 |
| 只在一个列表排名第 1 | ~0.016 |
| 只在一个列表排名第 20 | ~0.013 |
| 只在一个列表排名第 30 | ~0.011 |
